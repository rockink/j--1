...
enum TokenKind {
	/* KEYWORDS */
    		EOF("<EOF>"), ABSTRACT("abstract"), ASSERT("assert"), BOOLEAN("boolean"), BREAK("break"), BYTE("byte"), 
    		CASE("case"), CATCH("catch"), CHAR("char"), CLASS("class"), CONST("const"), CONTINUE("continue"), 
    		DEFAULT("default"), DO("do"), DOUBLE("double"), ELSE("else"), ENUM("enum"), EXTENDS("extends"),
    		FALSE("false"), FINAL("final"), FINALLY("finally"), FLOAT("float"), FOR("for"), GOTO("goto"), IF("if"),
    		IMPLEMENTS("implements"), IMPORT("import"), INSTANCEOF("instanceof"), INT("int"), INTERFACE("interface"), 
    		LONG("long"), NATIVE("native"), NEW("new"), NULL("null"), PACKAGE("package"), PRIVATE("private"), 
    		PROTECTED("protected"), PUBLIC("public"), RETURN("return"),SHORT("short"), STATIC("static"), 
    		STRICTFP("strictfp"), SUPER("super"), SWITCH("switch"), SYNCHRONIZED("synchronized"), THIS("this"), 
    		THROW("throw"), THROWS("throws"), TRANSIENT("transient"), TRUE("true"), TRY("try"), VOID("void"), 
    		VOLATILE("volatile"), WHILE("while"), 
    /* OPERATORS */		
    		ASSIGN("="), PLUS("+"), PLUS_ASSIGN("+="), MINUS("-"), 
    		MINUS_ASSIGN("-="), STAR("*"), STAR_ASSIGN("*="), DIV("/"), DIV_ASSIGN("/="), MOD("%"), MOD_ASSIGN("%=" ),
    		BAND("&"), BAND_ASSIGN("&="), BOR("|"), BOR_ASSIGN("|="), BXOR("^"), BXOR_ASSIGN("^="),
    		BNOT("~"), SL("<<"), SL_ASSIGN("<<="), SR(">>"), SR_ASSIGN(">>="), BSR(">>>"), BSR_ASSIGN(">>>="),
    		EQUAL("=="), NOT_EQUAL("!="), GT(">"), GE(">="), LT("<"), LE("<="), LAND("&&"), LOR("||"), LNOT("!"),
    		INC("++"), DEC("--"), QUESTION("?"), COLON(":"), ANNOTATION("@"), 
    /* SEPARATORS */		
    ...
    /* LITERALS */
    		INT_LITERAL("<INT_LITERAL>"), CHAR_LITERAL("<CHAR_LITERAL>"), STRING_LITERAL("<STRING_LITERAL>"),
